    ...
    async start({profile}) {
        ...
        inquirer.prompt([{
            ...
            inquirer.prompt([
                ...
                console.log(chalk.white(lsblkResult))
                ...
                inquirer.prompt([{
                    ...
                    if (answers.autoResizePartition) {
                        ...
                        console.log(chalk.yellowBright('Please make sure to manually expand the partition and file system!'))
...
async function _performDiskSizeChange(instance, volumeId, sizeGB, sshKeyPath) {
    ...
    /* let resultJson = Utils.parseExecResult(
     ...
     console.log(chalk.greenBright(`Successfully changed the size of volume [${volumeModification.VolumeId}] size from ${volumeModification.OriginalSize}GB to ${volumeModification.TargetSize}GB!`))
...
async function _resizePartition(instance, volumeId, sizeGB, sshKeyPath) {

    await exec(`sudo ssh -i ${sshKeyPath} -o StrictHostKeyChecking=no ubuntu@${instance.ipAddress} "sudo growpart /dev/nvme0n1 1"`)
    await exec(`sudo ssh -i ${sshKeyPath} -o StrictHostKeyChecking=no ubuntu@${instance.ipAddress} "sudo resize2fs /dev/nvme0n1p1"`)
    ...
    console.log(chalk.white(dfResult))
