 ...
 * @private
 */
async function _flow_confirmVolumeSizeChangeParameters(instance, volume, pathToSSHKey) {

    const answers = await inquirer.prompt([{
        type: 'confirm',
        name: 'performResize',
        message: `This will change Volume [${volume.id}] on instance [${instance.id} - ${instance.ipAddress}] from size [${volume.size}GB] to size [${REQUESTED_SIZE_GB}GB]. Continue?`
    }])
 ...
 * @private
 */
async function _flow_selectSshKey(instance, volume) {
...
async function _flow_selectVolume(instance) {
    ...
    if (_relevantVolumes.length === 0) {
        ...
        throw new Error('No volumes with size 8GB were found! cannot continue')
